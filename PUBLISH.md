...
发布后若想改用你自己的作用域名（如 `@<你>/dsh-wallpaper`），改名只需动 3 处并重建：

1. `package.json` → `"name"`（并把 `repository.url` 里的 `<your-github-username>` 换成你的）
...
git remote add origin https://github.com/<your-github-username>/dsh-wallpaper.git
