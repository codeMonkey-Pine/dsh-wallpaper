	...
	factory: (require) => {
		...
		*/
		...
		function rawUrl(id, relativePath) {
			...
			return `${API.raw}/${encodeURIComponent(id)}/${path}?v=2`;
		}
		//#endregion
		...
		*/
		...
		var SettingsStore = class {
			...
			applyLibrary(snapshot, error = null) {
			...
			*/
			async loadLibrary(api, force = false) {
...
`;
		...
		*/
		function VideoMedia(props) {
			...
			return react.default.createElement(react.default.Fragment, null, props.poster !== void 0 ? react.default.createElement("img", {
				src: props.poster,
				alt: "",
				draggable: false,
				style: props.style
		...
		function MediaElement(props) {
			...
			const previewUrl = entry.preview !== "" ? rawUrl(entry.id, entry.preview) : void 0;
		...
		function WallpaperLayer(props) {
			...
			const [library, setLibrary] = (0, react.useState)(() => store.getLibrary());
			const [libraryError, setLibraryError] = (0, react.useState)(() => store.getLibraryError());
			...
			const refreshedForRef = (0, react.useRef)(null);
			(0, react.useEffect)(() => {
				let alive = true;
				...
				store.loadLibrary(api, false);
				api.desiredState().then((desired) => {
					if (alive) store.mergeDesired(desired);
				}).catch(() => void 0);
				return () => {
					alive = false;
					unsubscribe();
		...
		*/
		...
		function WallpaperPanel(props) {
			...
			const [library, setLibrary] = (0, react.useState)(() => store.getLibrary());
			const [loading, setLoading] = (0, react.useState)(() => store.isLoadingLibrary());
			const [error, setError] = (0, react.useState)(() => store.getLibraryError());
			...
			(0, react.useEffect)(() => {
				if (library === null) store.loadLibrary(api, false);
			}, [api, store]);
			const rescan = (0, react.useCallback)(async () => {
